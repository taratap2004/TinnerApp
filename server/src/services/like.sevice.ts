...
export const LikeService = {
    togglelike: async function (user_id: string, target_id: string): Promise<boolean> {
        ...
        if (!target)
            throw new Error("Invalid traget_id")
    ...
    getFollowers: async function (user_id: string, pagination: userPagination): Promise<userPaginator> {
        const _query = User.findById(user_id)
            .populate({
                ...
                select: '_id username display_name photos introduction  interest location  gender daete_of_birtht',
    ...
    getFollowings: async function (user_id: string, pagination: userPagination): Promise<userPaginator> {
        const _query = User.findById(user_id)
            .populate({
                path: "following",
                match: { $and: QueryHelper.parseUserQuery(pagination) },
